...
class ThrowerAnt(Ant):
    ...
    damage = 1
    food_cost = 3
    upper_bound = None  # The maximum distance to throw a leaf
    lower_bound = None     # The minimum distance to throw a leaf
    ...
    def nearest_bee(self):
        ...
        if self.upper_bound is None and self.lower_bound is None:
            ...
            start = self.place
            distance = 0
            while not start.is_hive and distance <= self.upper_bound:
                if start.bees and distance >= self.lower_bound:
                    return random_bee(start.bees)
                else:
                    start = start.entrance
                    distance += 1
...
class HungryAnt(Ant):
    ...
    implemented = True
    def action(self, gamestate):
        """A HungryAnt eats a Bee in its Place, if it is not on cooldown."""
        if self.cooldown == 0:
            if self.place.bees:
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        if isinstance(other, ContainerAnt) or self.ant_contained is not None:
            return False
        else:
            return True
        # END Problem 8a
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        """A TankAnt attacks all Bees in its Place."""
        # BEGIN Problem 9
        for bee in self.place.bees.copy():
...
class QueenAnt(ThrowerAnt):
    ...
    # OVERRIDE CLASS ATTRIBUTES HERE
    def __init__(self, health=1):
        """Create a QueenAnt with a HEALTH quantity."""
        super().__init__(health)
    # BEGIN Problem 12
    implemented = True   # Change to True to view in the GUI
    # END Problem 12
        ...
        while location is not None:
            if location.ant is not None:
                if isinstance(location.ant, ContainerAnt):
                    location.ant.double()
                    if location.ant.ant_contained is not None:    
