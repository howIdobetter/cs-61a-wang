...
def balanced(m):
    ...
    if is_planet(m):
        return True
    if not is_mobile(m):
        return False  # 题目保证输入是 mobile 或 planet，但防御性编程
...
def berry_finder(t):
    ...
    if label(t) == 'berry':  # 检查当前节点是否是 'berry'
        return True
    for branch in branches(t):  # 遍历所有子树
        if berry_finder(branch):  # 递归检查子树
            return True
    return False  # 所有子树都没有 'berry'
...
def max_path_sum(t):
    ...
    for branch in branches(t):
        max_count = max(max_count, max_path_sum(branch) + label(t))
    return max_count
